...
export function VoiceControl({ canvasOpen, onCanvasMinimize, onCanvasToggle, sessionId, items, running, onSend, onAbort, stageTarget, onModeChange, title, browserAvailable, browserActivity, terminalActivity, toolEvents }: {
  ...
  // True only when the user *manually* un-mutes while the agent is playing:
  // that is the explicit barge-in (interrupt) intent. Speech detected during
  // the automatic re-arm window after a phrase is treated as possible echo.
  const bargeArmed = useRef(false);
  const rearmTimer = useRef<ReturnType<typeof setTimeout>>();
  ...
  const toggleMute = async () => {
    ...
    try {
      if (next) {
        ...
        clearTimeout(rearmTimer.current);
        bargeArmed.current = speakingRef.current;
  ...
  const synthesize = async (text: string, signal: AbortSignal, audio: AudioContext, kind:'reply'|'status'='reply') => {
    ...
    let speechStream: Awaited<ReturnType<typeof preparePcmSpeech>> | undefined;
    if (response.headers.get("content-type")?.startsWith("audio/pcm")) {
      ...
      if (!sequential.current && response.headers.get("x-voice-streaming") === "true") speechStream = await preparePcmSpeech(body!, audio, signal);
    ...
    mark('audio_ready');
    // Half-duplex gate: blind the microphone while agent audio plays so its
    // output can never be re-transcribed and re-prompted (the echo loop that
    // made the agent answer its own voice). Two barriers: the VAD is paused
    // (frames drop, no speech events fire) and the mic track itself is
    // disabled. Re-armed shortly after playback ends — or immediately if a
    // manual mute is active, which already keeps the mic closed.
    const rearmMic = () => {
      if (!mounted.current || mutedRef.current || speakingRef.current) return;
      levels.current.input = 0;
      const detector = vad.current;
      if (detector) {
        detector.setOptions({ submitUserSpeechOnPause: true });
    ...
    const armMic = () => {
      ...
      setSpeaking(true);
      console.debug("[pith-voice-gate]", "arm (mic blind while agent speaks)", { muted: mutedRef.current });
      if (mutedRef.current) return; // manual mute stays authoritative
      void vad.current?.pause().catch(() => {});
      stream.current?.getTracks().forEach(track => { track.enabled = false; });
    };
    const disarmMic = () => {
      ...
      console.debug("[pith-voice-gate]", "disarm (re-arming after reverb tail)", { muted: mutedRef.current });
      if (mutedRef.current) return; // stays closed until the user un-mutes
      // Hold the mic closed across the 40ms inter-phrase gaps and the speaker's
      // reverb tail (~250ms) before listening again; a new phrase cancels this.
      rearmTimer.current = setTimeout(rearmMic, 250);
    };
    ...
    return Object.assign(play, { completed: speechStream?.completed });
  };
  const start = async () => {
    ...
    try {
      ...
      const detector = await detectorModule.MicVAD.new({
        ...
        onSpeechStart: () => { if (current() && !mutedRef.current && !speakingRef.current && !latest.current.compacting) {if(profiling.current)profiler.current!.begin();live.begin();} },
        onVADMisfire: () => { if (current()) {live.discard();if(profiling.current)profiler.current!.close('vad_misfire');} },
        onFrameProcessed: (probabilities, frame) => {
          if(current()&&!mutedRef.current&&!speakingRef.current&&profiling.current&&probabilities.isSpeech>=0.35)profiler.current!.lastSpeech();
          if (current() && !mutedRef.current && !speakingRef.current && !latest.current.compacting) live.frame(probabilities.isSpeech, frame);
          if (current() && !mutedRef.current && !speakingRef.current) levels.current.input = Math.min(1, Math.sqrt(frame.reduce((sum, value) => sum + value * value, 0) / frame.length) * 7);
        },
        onSpeechRealStart: () => {
          if (!current() || mutedRef.current) return;
          // While the agent is playing, only an explicit user un-mute counts
          // as a barge-in; auto re-arm windows must not interrupt (echo guard).
          if (speakingRef.current && !bargeArmed.current) return;
          bargeArmed.current = false;
