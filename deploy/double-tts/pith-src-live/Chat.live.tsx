...
import { isCompact, subscribeCompact } from "../compact-mode";
...
export function Chat({
  ...
  /** Small displays only: start a fresh chat (named from the time). */
  onCreateNew?: () => Promise<void>;
}) {
  const [input, setInput] = useState("");
  const [voiceMode, setVoiceMode] = useState(false);
  // Small-display layout, kept in step with the compact store so the New Chat
  // button appears and retires with it without a full page re-render.
  const [compactView, setCompactView] = useState(isCompact());
  useEffect(() => subscribeCompact(() => setCompactView(isCompact())), []);
  const [newBusy, setNewBusy] = useState(false);
  const newChat = async () => {
    if (newBusy || !onCreateNew) return;
    setNewBusy(true);
    try {
      await onCreateNew();
    } finally {
  ...
  return (
    <div className="session-workspace relative flex h-full min-h-0 flex-col">
      ...
      <BrainBar sessionId={session.id} session={session} />
      <PermissionBar active={session.provider === "hermes"} />
      {compactView && (
        <div className="compact-toprow">
          <NewChatButton busy={newBusy} onClick={newChat} />
          <span className="compact-title" title={session.title}>{session.title}</span>
        </div>
      )}
      ...
      <header className={voiceMode ? "hidden" : "border-b border-line px-4 py-3"}>
        ...
        {/* On compact displays the session title lives in the top row instead
            — this would otherwise compete with the New Chat button for space. */}
        <div className={compactView ? "hidden" : "min-w-0"}>
        ...
        />
          <ComposerBar
            ...
            actions={<>
              ...
              <SpeakReplies sessionId={session.id} items={items} voiceActive={voiceMode} />
